...
class SocketProvider extends GetxController {
  ...
  void connectSocket() {
    ...
    webSocket = IOWebSocketChannel.connect(Uri.parse(SocketConstants.baseUrl),
        headers: mapObj);
    _disconnected = false;
  ...
  parseMessage(SocketResponse socketResponse) {
    printFunction("webSocket event",
        "${socketResponse.channel} : ${socketResponse.event}");
    ...
    switch (socketResponse.event) {
      ...
      case SocketConstants.eventNotification:
        NotificationData data =
            notificationDataFromJson(socketResponse.data ?? "");
        _callListeners(socketResponse.channel ?? "",
            SocketConstants.eventNotification, data);
        return;
      case SocketConstants.eventOrderPlace:
        if ((socketResponse.channel ?? "")
            .contains(SocketConstants.channelDashboard)) {
          final sData = socketOrderPlaceFromJson(socketResponse.data ?? "");
          _callListeners(socketResponse.channel ?? "",
              SocketConstants.eventOrderPlace, sData);
        }
        return;
      case SocketConstants.eventOrderRemove:
        if ((socketResponse.channel ?? "")
          ...
          final sData = socketOrderPlaceFromJson(socketResponse.data ?? "");
          _callListeners(socketResponse.channel ?? "",
              SocketConstants.eventOrderRemove, sData);
        }
        return;
      case SocketConstants.eventProcess:
        SocketTradeInfo data =
            socketTradeInfoFromJson(socketResponse.data ?? "");
        _callListeners(
            socketResponse.channel ?? "", SocketConstants.eventProcess, data);
        return;
    }
    ...
    if (socketResponse.event == selfEvent) {
      ...
      _callListeners(
          socketResponse.channel ?? "", socketResponse.event ?? "", sData);
      return;
    }
  }
