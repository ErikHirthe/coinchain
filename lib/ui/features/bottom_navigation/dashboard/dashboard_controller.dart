...
import 'package:tradexpro_flutter/data/models/trade_option_response.dart';
import 'package:tradexpro_flutter/data/models/trade_result_response.dart';
import 'package:tradexpro_flutter/data/models/trade_return_response.dart';
...
class DashboardController extends GetxController
  ...
  Rx<TradeOptionResponse> tradeOptions = TradeOptionResponse().obs;
  Rx<TradeHistoryResponse> tradeHistorys = TradeHistoryResponse().obs;
  Rx<UserTradeReturnReponse> userTradeReturnResponse =
      UserTradeReturnReponse().obs;
  Rx<UserTradeResultReponse> userTradeResultResponse =
      UserTradeResultReponse().obs;
  RxList<TradeHistory> usertradeHistoryList = <TradeHistory>[].obs;
  ...
  // void updateTradeCurrentPrice({String? currentPrice}) {
  //   tradeCurrentPrice.value = "${DateTime.now()}";
  //   // Decrease secondsLeft by 1
  //   secondsLeft.value = secondsLeft.value - 1;
  //   if (secondsLeft.value > 0) {
  //     // Repeat the update every second until 30 seconds
  //     Future.delayed(const Duration(seconds: 1), updateTradeCurrentPrice);
  //   }
  // }
  ...
  void onDataGet(channel, event, data) {
    if (channel == channelDashboard) {
      if ((event == SocketConstants.eventOrderPlace ||
        ...
        final selfEvent =
            "${SocketConstants.eventOrderPlace}_${gUserRx.value.id}";
        if (event == selfEvent && data is SocketUserHistory) {
          ...
          if (list != null) {
            tradeHistoryList.value =
                (list.length >= 5) ? list.sublist(0, 5) : list;
          }
        }
      }
  ...
  void subscribeCoinPairChannel() {
    ...
    if (selectedCoinPair.value.parentCoinId != null) {
      channelDashboard =
          "${SocketConstants.channelDashboard}${selectedCoinPair.value.parentCoinId}-${selectedCoinPair.value.childCoinId}";
      APIRepository().subscribeEvent(channelDashboard, this);
      channelTradeInfo =
          "${SocketConstants.channelTradeInfo}${selectedCoinPair.value.parentCoinId}-${selectedCoinPair.value.childCoinId}";
  ...
  void unSubscribeChannel(bool isDispose) {
    if (channelDashboard.isValid) {
      APIRepository()
          .unSubscribeEvent(channelDashboard, isDispose ? this : null);
    }
    if (channelTradeInfo.isValid) {
      APIRepository()
          .unSubscribeEvent(channelTradeInfo, isDispose ? this : null);
    }
    if (channelUserTrades.isValid) {
      APIRepository()
          .unSubscribeEvent(channelUserTrades, isDispose ? this : null);
    }
    channelTradeInfo = "";
    channelUserTrades = "";
    channelDashboard = "";
      ...
      if (resp.success) {
        ...
        if (selectedCoinPair.value.coinPair == null) {
          ...
          if (exPair.isNotEmpty) {
            selectedCoinPair.value = (dashboardData.value.coinPairs ?? [])
                .firstWhere((element) => element.coinPair == exPair);
          }
        }
        ...
        Future.delayed(const Duration(milliseconds: 250),
            () => getTradeHistoryList(FromKey.buySell));
        Future.delayed(
            const Duration(milliseconds: 500), () => getExchangeTradeList());
  ...
  void getExchangeOrderList(String type) {
    APIRepository()
        ...
        .then((resp) {
      if (resp.success) {
        var list = List<ExchangeOrder>.from(resp.data[APIKeyConstants.orders]
    ...
    if (list != null) {
      if (type == FromKey.sell) {
        ...
        sellExchangeOrder.value =
            (list.length >= 15) ? list.sublist(0, 15) : list;
      } else {
  ...
  void getExchangeTradeList() {
    APIRepository()
        .getExchangeTradeList(dashboardData.value.orderData?.baseCoinId ?? 0,
            dashboardData.value.orderData?.tradeCoinId ?? 0)
        .then((resp) {
      if (resp.success) {
        final list = List<ExchangeTrade>.from(resp
            .data[APIKeyConstants.transactions]
  ...
  //TradeOptions
  void getTradeOptions() {
    APIRepository().getTradeOptions().then((response) {
      if (response.success) {
        tradeOptions.value = TradeOptionResponse.fromJson(response.data);
      }
    }, onError: (err) {
      showToast(err.toString());
    });
  ...
  void postUserTrade(
      ...
      required int? tradeOptionID,
      required String tradeOptionDuration,
      required String tradeOptionDurationForshow,
    ...
    APIRepository()
        .userTrade(
            ...
            tradeOptionDuration: tradeOptionDuration,
            tradeOptionDurationForshow: tradeOptionDurationForshow,
        ...
        .then((response) {
      if (response.success) {
        userTradeReturnResponse.value =
            UserTradeReturnReponse.fromJson(response.data);
      }
    }, onError: (err) {
      showToast(err.toString());
    });
  ...
  void postUserTradeResult({String? uuid, num? currentPrice}) {
    APIRepository()
        .userTradeResult(uuid: uuid, current_price: currentPrice)
        .then((response) {
      if (response.success) {
        debugPrint("user trade result ==> ${response.data}");
        userTradeResultResponse.value =
            UserTradeResultReponse.fromJson(response.data);
      }
    });
  }

  // void getUserTradeHistory({String? tradeHistoryStatus}) {
  //   if (gUserRx.value.id == 0) return;
  //   isHistoryLoading.value = true;
  //   tradeHistorys.value.data = [];
  //   APIRepository()
  //       .userTradeHistory(tradeHistoryStatus: tradeHistoryStatus)
  //       .then((response) {
  //     if (response.success) {
  //       if (tradeHistorys.value.data!.isNotEmpty) {
  //         tradeHistorys.value.data?.clear();
  //       }
  //       isHistoryLoading.value = false;

  //       tradeHistorys.value = TradeHistoryResponse.fromJson(response.data);
  //     }
  //   }, onError: (err) {
  //     isHistoryLoading.value = false;
  //     showToast(err.toString());
  ...
  void getTradeHistoryList(String orderType) {
    ...
    APIRepository()
        .getTradeHistoryList(
            orderData?.baseCoinId ?? 0, orderData?.tradeCoinId ?? 0, orderType)
        .then((resp) {
      isHistoryLoading.value = false;
      if (resp.success) {
        final key = orderType == FromKey.trade
            ? APIKeyConstants.transactions
            : APIKeyConstants.orders;
        final list =
            List<Trade>.from(resp.data[key].map((x) => Trade.fromJson(x)));
  ...
  void getCoinPairList(String searchText) {
    if (searchText.isEmpty) {
      ...
      final list = (dashboardData.value.coinPairs ?? [])
          .where((element) =>
              (element.coinPairName ?? "").toLowerCase().contains(searchText))
          .toList();
  ...
  void placeOrderStopMarket(bool isBuy, int baseCoinId, int tradeCoinId,
      double amount, double limit, double stop, Function() onSuccess) {
    showLoadingDialog();
    APIRepository()
        .placeOrderStopMarket(
            isBuy, baseCoinId, tradeCoinId, amount, limit, stop)
        .then((resp) {
      hideLoadingDialog();
      if (resp.success) {
        final success = resp.data[APIKeyConstants.status] as bool? ?? false;
  ...
  void cancelOpenOrderApp(Trade trade) {
    showLoadingDialog();
    APIRepository().cancelOpenOrderApp(trade.type ?? "", trade.id ?? 0).then(
        (resp) {
      hideLoadingDialog();
      if (resp.success) {
        final success = resp.data[APIKeyConstants.status] as bool? ?? false;
  ...
  final WebSocketChannel socketChannel = WebSocketChannel.connect(
    Uri.parse("ws://staging.coins-chain.com:8090"),
  );

  sendSubscribe() {
    socketChannel.sink.add(jsonEncode({"command": "pendingTrades"}));
  }

  sendUnsubscribe() {
    socketChannel.sink.add(jsonEncode({"command": "unsubscribe"}));
