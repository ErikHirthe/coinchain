...
class MyProfileController extends GetxController {
  ...
  Rx<User> userProfile = User(id: 2).obs;
  // Rx<AvailableBalanceResponse> availableBalanceResponse =
  //     AvailableBalanceResponse().obs;
  ...
  void getUserActivities(Function(List<UserActivity>) onSuccess) {
    APIRepository().getSelfProfile().then((resp) {
      if (resp.success) {
        ...
        final list = List<UserActivity>.from(
            listMap.map((x) => UserActivity.fromJson(x)));
        onSuccess(list);
  ...
  void getUserProfile() {
    APIRepository().getSelfProfile().then((resp) {
      if (resp.success) {
        userProfile.value = User.fromJson(resp.data);
        debugPrint("Profile ==> ${resp.data}");
      }
    }, onError: (err) {
      showToast(err.toString());
    });
  ...
  void uploadDocuments(IdVerificationType type, File frontFile, File backFile,
      File selfieFile, Function(KycDetails) onSuccess) {
    showLoadingDialog();
    APIRepository()
        .uploadIdVerificationFiles(type, frontFile, backFile, selfieFile)
        .then((resp) {
      hideLoadingDialog();
      if (resp.success) {
        final success = resp.data[APIKeyConstants.success] as bool? ?? false;
  ...
  void deleteAccountRequest(
      String reason, String password, Function() onSuccess) {
    showLoadingDialog();
  ...
  // void userAvailableBalance() {
  //   APIRepository().getUserAvailableBalance().then((resp) {
  //     if (resp.success) {
  //       availableBalanceResponse.value =
  //           AvailableBalanceResponse.fromJson(resp.data);
  //     }
  //   }, onError: (err) {
  //     showToast(err.toString());
  //   });
