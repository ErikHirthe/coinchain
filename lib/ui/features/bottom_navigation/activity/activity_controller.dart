...
class ActivityScreenController extends GetxController {
  ...
  Map<String, String> getTypeMap() {
    ...
    var map = {
      ...
      HistoryType.userTrade: "Trade History".tr,
      ...
      HistoryType.userMining: "Lock-up Mining History".tr
    };
    ...
    map[HistoryType.refEarningWithdrawal] =
        "Referral Earning From Withdrawal".tr;
    ...
    APIRepository().getActivityList(loadedPage, type).then((resp) {
      ...
      if (resp.success) {
        if (type == HistoryType.fiatDeposit ||
            ...
            type == HistoryType.userTrade) {
          debugPrint("History ==> ${resp.data}");
          listResponse = ListResponse.fromJson(resp.data);
        } else if (type == HistoryType.userMining) {
          listResponse = ListResponse.fromJson(resp.data);
        } else {
          debugPrint("History 1 ==> ${resp.data}");
        ...
        if (listResponse != null) {
          ...
          if (type == HistoryType.swap) {
            list = List<SwapHistory>.from(
                listResponse.data!.map((x) => SwapHistory.fromJson(x)));
          } else if (type == HistoryType.userTrade) {
            list = List<TradeHistory>.from(
                listResponse.data!.map((x) => TradeHistory.fromJson(x)));
          } else if (type == HistoryType.buyOrder ||
              type == HistoryType.sellOrder ||
              type == HistoryType.transaction ||
              type == HistoryType.stopLimit) {
            list = List<Trade>.from(
                listResponse.data!.map((x) => Trade.fromJson(x)));
          } else if (type == HistoryType.fiatDeposit ||
              type == HistoryType.fiatWithdrawal) {
            list = List<FiatHistory>.from(
                listResponse.data!.map((x) => FiatHistory.fromJson(x)));
          } else if (type == HistoryType.refEarningTrade ||
              type == HistoryType.refEarningWithdrawal) {
            list = List<ReferralHistory>.from(
                listResponse.data!.map((x) => ReferralHistory.fromJson(x)));
          } else if (type == HistoryType.userMining) {
            list = List<MiningHistory>.from(
                listResponse.data!.map((x) => MiningHistory.fromJson(x)));
          } else {
            list = List<History>.from(
