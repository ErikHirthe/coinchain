...
class FiatDepositController extends GetxController {
  ...
  Rx<CryptoWalletResponse> cryptoWalletResponse = CryptoWalletResponse().obs;
    ...
    APIRepository().getFAQList(1, type: FAQType.deposit).then((resp) {
      if (resp.success) {
        ...
        if (response.data != null) {
          List<FAQ> list =
              List<FAQ>.from(response.data!.map((x) => FAQ.fromJson(x)));
          faqList.value = list;
        }
      }
    }, onError: (err) {});
  ...
  Future<void> getCurrencyDepositRate(
      int walletId, double amount, Function(double) onRate,
      {String? currency, int? bankId, int? walletIdFrom}) async {
    final pMethod =
        fiatDepositData.value.paymentMethods?[selectedMethodIndex.value];
    APIRepository()
        .getCurrencyDepositRate(walletId, pMethod?.id ?? 0, amount,
            currency: currency, bankId: bankId, walletIdFrom: walletIdFrom)
        .then((resp) {
      if (resp.success) {
  ...
  Future<void> currencyDepositProcess(
    int walletId,
    double amount,
    Function() onSuccess, {
    String? currency,
    ...
    final pMethod =
        fiatDepositData.value.paymentMethods?[selectedMethodIndex.value];
    debugPrint("pMethod value ${selectedMethodIndex.value}");
    if ((getSettingsLocal()?.currencyDeposit2FaStatus ?? "") != "1") {
      ...
      depositProcess(
        walletId, pMethod?.id ?? 0, amount, onSuccess,
        currency: currency,
        //bankId: bankId,
        remark: remark,
        file: file,
  ...
  List<String> getMethodList(FiatDeposit? fiatDepositData) {
    if (fiatDepositData?.paymentMethods.isValid ?? false) {
      return fiatDepositData!.paymentMethods!
          .map((e) => e.title ?? "")
          .toList();
    }
    return [];
  ...
  void getCryptoWallets() {
    showLoadingDialog();
    APIRepository().cryptoWallets().then((response) {
      if (response.success) {
        hideLoadingDialog();
        cryptoWalletResponse.value =
            CryptoWalletResponse.fromJson(response.data);
      }
    }, onError: (err) {
      isLoading.value = false;
      showToast(err.toString());
