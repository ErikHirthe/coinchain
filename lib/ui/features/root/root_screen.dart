...
import 'package:tradexpro_flutter/ui/features/bottom_navigation/activity/report_screen.dart';
...
class RootScreenState extends State<RootScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    keyboardSubscription = keyboardVisibilityController.onChange
        .listen((bool visible) => isKeyBoardShowing.value = visible);
      ...
      tabBuilder: (int index, bool isActive) {
        ...
        return Container(
            ...
            child: Icon(iconList[index],
                size: index == 2 ? Dimens.iconSizeMid : Dimens.iconSizeMin,
                color: iconColor));
      },
        ...
        child: Drawer(
            ...
            child: SafeArea(
              child: Obx(() {
                ...
                return ListView(
                  padding:
                      const EdgeInsets.only(top: Dimens.paddingMainViewTop),
                  shrinkWrap: true,
                  children: [
                    Row(
                      children: [
                    ...
                    if (hasUser)
                      _profileView(gUserRx.value)
                    else
                      signInNeedView(isDrawer: true),
                    vSpacer20(),
                    if (hasUser) _loggedInMenuView() else _loggedOutMenuView(),
                  ],
                );
              }),
            )),
  ...
  Widget _profileView(User user) {
    return Column(
      ...
      children: [
        ...
        vSpacer10(),
        textAutoSizeKarla(getName(user.firstName, user.lastName),
            fontSize: Dimens.regularFontSizeLarge),
        vSpacer5(),
        textAutoSizeKarla(user.email ?? "",
            fontSize: Dimens.regularFontSizeMid, fontWeight: FontWeight.normal),
      ],
    );
  }

  Widget _loggedOutMenuView() {
    return Column(
      children: [
        // drawerNavMenuItem(
        //     navTitle: 'Staking'.tr,
        //     iconPath: AssetConstants.icStaking,
        //     navAction: () => Get.to(() => const StakingScreen())),
        if (getSettingsLocal()?.enableGiftCard == 1)
          drawerNavMenuItem(
  ...
  Widget _loggedInMenuView() {
    ...
    return Column(
      children: [
        ...
        if (settings?.liveChatStatus == "1")
          drawerNavMenuItem(
              navTitle: 'Support'.tr,
              iconPath: AssetConstants.icMessage,
              navAction: () => Get.to(() => const CrispChat())),
        // drawerNavMenuItem(
        //     navTitle: 'Staking'.tr,
        //     iconPath: AssetConstants.icStaking,
        //     navAction: () => Get.to(() => const StakingScreen())),
