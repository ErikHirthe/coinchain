...
class SignInController extends GetxController {
  ...
  void loginUser(BuildContext context) {
    showLoadingDialog();
    APIRepository()
        .loginUser(emailEditController.text.trim(), passEditController.text)
        .then((resp) {
      hideLoadingDialog();
      if (resp.success) {
        ...
        if (success) {
          ...
          if (verify != null && verify == 0) {
            alertForAction(context,
                title: "Need verification".tr,
                subTitle: "verify unverified account".tr,
                buttonTitle: "Verify".tr, onOkAction: () {
              Get.back();
              Get.off(() => EmailVerifyPage(
                  registrationId: emailEditController.text.trim()));
            });
          } else {
            showToast(message);
          }
  ...
  void verifyGoogleCodeLogin(BuildContext context, int userId) {
    TwoFAHelper.get2FACode(forText: "Sign In".tr, buttonText: "Sign In".tr,
        (code) {
      hideKeyboard(context);
      showLoadingDialog();
  ...
  void handleLoginSuccess(dynamic resp) {
    GetStorage().write(
        PreferenceKey.accessToken, resp[APIKeyConstants.accessToken] ?? "");
    GetStorage().write(
        PreferenceKey.accessType, resp[APIKeyConstants.accessType] ?? "");
